...
class RunStepsValidation : public edm::EDAnalyzer {
...
public:
    ...
    explicit RunStepsValidation(const edm::ParameterSet&);
    ~RunStepsValidation();
...
private:
    ...
    edm::InputTag rechit_, cluster_;
    bool rechitActive_, clusterActive_;
...
RunStepsValidation::RunStepsValidation(const edm::ParameterSet& iConfig) {

   //Get the information which is stored inside the configuration file!
   rechit_ = iConfig.getParameter<edm::InputTag>("rechit");
    cluster_ = iConfig.getParameter<edm::InputTag>("cluster");
    rechitActive_ = iConfig.getParameter<bool>("rechitUsed");
    clusterActive_ = iConfig.getParameter<bool>("clusterUsed");
...
RunStepsValidation::~RunStepsValidation() { }

void RunStepsValidation::beginJob() {
    createHistograms(19);
}

void RunStepsValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    iEvent.getByLabel(cluster_, clusterLinks);
    ...
    // Get the clusters
    if(clusterActive_ == true){
    
    	lookAtClusters();
    }
 
   //Get the RecHits
   if(rechitActive_ ==  true){
   
   	lookAtRecHits();
   	//Move these lines to the lookAtRecHits class !
    ...
    if(verbosity>1) std::cout << std::endl << "-- Number of SimHits in the event : " << nHits << std::endl;
    
}

void RunStepsValidation::getClusterInformation(cluster, iPos){    //Currently just a copy of whatever was defined in the RunStepsClusterValidation file --> TO CHECK!!
...
void RunStepsValidation::lookAtClusters(){                                                                 //--> Should have similar class for lookAtRechits!

    edm::Handle< SiPixelClusterCollectionNew > edmClusters;
    iEvent.getByLabel(cluster_, edmClusters);
    const edmNew::DetSetVector<SiPixelCluster>& pixelClusters = *edmClusters;

    //*********************************//
    // loop over the cluster collection //
    //*********************************//
    ...
    for (DSViter = pixelClusters.begin(); DSViter != pixelClusters.end(); DSViter++) {
        ...
        for (cu = DSViter->begin(); cu != DSViter->end(); ++cu) {
        	...
        	// Pass this cluster to the class to get all the detailed information about the clusters
        	//  --> Should be the same class for both the RecHit validation as for the Cluster validation!!
        	// ~~> Additional information which should be passed on is the iPos value!!
        	getClusterInformation(cluster, iPos);
        
            ++nClusters;    //--> Is this still used anywhere?
        }
...
////   ---------> STILL TO UPDATE!!!!!!!!!!!
    ...
    if(verbosity>1) std::cout << std::endl << "-- Enter loop over links" << std::endl;
	...
	if(verbosity>1) std::cout << std::endl << std::endl << "--- DetId=" << rawid << std::endl;
	    ...
	    if(verbosity>1) std::cout << std::endl << "---- Cluster size="  << cluster->size()    << " | " << sizeLink << " SimTracks: ids=(" ;
		...
		if(verbosity>1) std::cout << "----- SimHit #" << iH
				   << " type="    << simh_type
		  //<< " s_id="    << simh_detid
				   << " s_lay="   << simh_layer
				   << " c_lay="   << layer
	      ...
	      if(!found_hits && verbosity>1) std::cout << "----- FOUND NO MATCHED HITS" << std::endl;
    ...
    if(verbosity>1) std::cout << "- Enter efficiency computation" << std::endl;
    ...
    if(verbosity>1) std::cout << "- loop over map of hits & clusters (size=" << map_hits.size() << ")" << std::endl;

    for( iMapHits=map_hits.begin() ; iMapHits!=map_hits.end() ; iMapHits++) {

      if(verbosity>1) std::cout << "-- SimTrack ID=" << iMapHits->first << std::endl;
	...
	if(verbosity>1) std::cout << " DetId=" << theHit_id
			   << " Layer=" << theHit_layer
			   << " Type="  << theHit_type
			   << std::endl;
	...
	if(theHit_layer==0) {
	  if(verbosity>1) std::cout << "---- !! layer=0 !!" << std::endl;
	  continue;
	}

	// Clusters matched to the SimHit
	if(verbosity>2) std::cout << "--- Getting corresponding clusters" << std::endl;
	matched_clusters = ((iMapHits->second)[iH]).second;
	nMatchedClusters = matched_clusters.size();

	// Find layer in map of histograms
	if(verbosity>2) std::cout << "--- Find layer=" << theHit_layer << " in map of histograms" << std::endl;
	iPos = layerHistoMap.find(theHit_layer);
        if (iPos == layerHistoMap.end()) {
	  if(verbosity>2) std::cout << "---- add layer in the map" << std::endl;
	...
	if( nMatchedClusters==0 ) {
	  if(verbosity>1) std::cout << "---- No Cluster Matched" << std::endl;
	}
	else {
	  if(verbosity>1) std::cout << "---- Yes Cluster Matched = " << nMatchedClusters << std::endl;
	}

	if( map_effi.find(theHit_layer)==map_effi.end() )
	  for(int iT=0 ; iT<nTypes ; iT++) {
	    map_effi[theHit_layer].push_back(init_counter);
	    if(verbosity>2) std::cout << "----- type #" << iT << " layer=" << theHit_layer << " map size=" << map_effi.size() << std::endl;
	  }
    ...
    if(verbosity>1) std::cout << "- fill [per layer] effi histo from effi map (size=" << map_effi.size() << ")" << std::endl;
    for( iMapEffi=map_effi.begin() ; iMapEffi!=map_effi.end() ; iMapEffi++ ) {

      iPos = layerHistoMap.find(iMapEffi->first);
      if(verbosity>1) std::cout << "-- layer=" << iMapEffi->first << std::endl;
	...
	if(verbosity>1) std::cout << "--- type #"   << iT
    ...
    if( countHit != nHits )
      if(verbosity>1) std::cout << "---- Missing hits in the efficiency computation : "
			 << countHit << " != " << nHits << std::endl;

    if(verbosity>999) std::cout << nTotalHits << nMatchHits << efficiency << std::endl;
}

void RunStepsValidation::endJob() { }

// Create the histograms
void RunStepsValidation::createLayerHistograms(unsigned int ival) {
...
void RunStepsValidation::createHistograms(unsigned int nLayer) {
    edm::Service<TFileService> fs;
    fs->file().cd("/");
...
unsigned int RunStepsValidation::getLayerNumber(const TrackerGeometry* tkgeom, unsigned int& detid) {
...
unsigned int RunStepsValidation::getLayerNumber(unsigned int & detid) {
    unsigned int layer = 999;
    DetId theDetId(detid);
...
int RunStepsValidation::isPrimary(const SimTrack& simTrk, edm::Handle<edm::PSimHitContainer>& simHits) {
    int result = -1;
...
int RunStepsValidation::isPrimary(const SimTrack& simTrk, const PSimHit& simHit) {
    int result = -1;
    unsigned int trkId = simTrk.trackId();
    if (trkId > 0) {
        int vtxIndx = simTrk.vertIndex();
...
DEFINE_FWK_MODULE(RunStepsValidation);
