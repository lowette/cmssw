// -*- C++ -*-
//
// Package:    RunStepsDigitizer
// Class:      RunStepsDigitizer
//
/**\class RunStepsDigitizer RunStepsDigitizer.cc RunSteps/Digitizer/src/RunStepsDigitizer.cc
...
// user include files
#include "RunSteps/Digitizer/interface/RunStepsDigitizer.h"
...
namespace cms
{
  RunStepsDigitizer::RunStepsDigitizer(const edm::ParameterSet& iConfig, edm::EDProducer& mixMod):
    first(true),
    _pixeldigialgo(),
    ...
    geometryType(iConfig.getParameter<std::string>("GeometryType"))
  {
            ...
            std::cout << "------------------------------------------------------------" << std::endl
                      << "-- Running RunSteps Digitizer v0.0" << std::endl
                      << "------------------------------------------------------------" << std::endl;
    ...
    if ( ! rng.isAvailable()) {
      throw cms::Exception("Configuration")
        << "RunStepsDigitizer requires the RandomNumberGeneratorService\n"
        "which is not present in the configuration file.  You must add the service\n"
        "in the configuration file or remove the modules that require it.";
    }

    rndEngine       = &(rng->getEngine());
    _pixeldigialgo.reset(new RunStepsDigitizerAlgorithm(iConfig,(*rndEngine)));

  }

  RunStepsDigitizer::~RunStepsDigitizer(){
  ...
  void
  RunStepsDigitizer::accumulatePixelHits(edm::Handle<std::vector<PSimHit> > hSimHits) {
    if(hSimHits.isValid()) {
  ...
  void
  RunStepsDigitizer::initializeEvent(edm::Event const& e, edm::EventSetup const& iSetup) {
    if(first){
  ...
  void
  RunStepsDigitizer::accumulate(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
    // Step A: Get Inputs
  ...
  void
  RunStepsDigitizer::accumulate(PileUpEventPrincipal const& iEvent, edm::EventSetup const& iSetup) {
    // Step A: Get Inputs
    for(vstring::const_iterator i = trackerContainers.begin(), iEnd = trackerContainers.end(); i != iEnd; ++i) {
      edm::Handle<std::vector<PSimHit> > simHits;
      edm::InputTag tag(hitsProducer, *i);
  ...
  void
  RunStepsDigitizer::finalizeEvent(edm::Event& iEvent, const edm::EventSetup& iSetup) {
